package thesis.three;


import java.util.Vector;

import peeremu.config.Configuration;
import peeremu.core.CommonState;
import peeremu.core.Control;
import peeremu.core.Descriptor;
import peeremu.core.Network;
import peeremu.core.Node;

public class GroupControl implements Control
{
  private int pid;
  private boolean first=true;
  private int maxsize;
  private long endtime;
  private int printgroup;
  private int printsusp;

  private static final String PAR_MAXSIZE="maxsize";
  private static final String PAR_ENDTIME="endtime";
  private static final String PAR_PRINTGROUP="printgroup";
  private static final String PAR_PRINTSUSP="printsusp";
  private Vector v = new Vector();
  private int outputsize=201;
  private int[] output=new int[outputsize];
  public GroupControl(String prefix)
  {
    pid = Configuration.getPid(prefix+"."+PAR_PROTOCOL);
    maxsize = Configuration.getInt(prefix+"."+PAR_MAXSIZE);
    endtime = Configuration.getLong(prefix+"."+PAR_ENDTIME);
    printgroup = Configuration.getInt(prefix+"."+PAR_PRINTGROUP);;
    printsusp = Configuration.getInt(prefix+"."+PAR_PRINTSUSP);    
  }
  @Override
  public boolean execute()
  {
    int i,j,k,ngroups=0;

    if(first)
    {
      for(i=0;i<outputsize;i++)
      {
        output[i]=0;
      }

      for(i=0;i<Network.size();i++)
      {
        Node n = Network.get(i);
        long myid = n.getDescriptor(pid).getID();
        RobustnessProtocol ap = (RobustnessProtocol)n.getProtocol(pid);
        ngroups = (int)Network.size()/ap.groupsize;
        if(ngroups==0)
          ngroups=1;
        int groupid = (int)myid%ngroups; 
        for(j=groupid;j<Network.size();j+=ngroups)
        {
          Node n1=Network.get(j); 
          if(j!=i)
          { 
            Suspicion susp = new Suspicion();
            susp.desc = n1.getDescriptor(pid);
            susp.suspvalue=0;
            susp.esttime=0;
            susp.lastmsgtime=0;
            susp.first=true;
            susp.initval=-1;
            susp.value=-1;
            susp.state = Suspicion.State.ALIVE;

            ap.sp.add(susp);
            GroupDetails det = new GroupDetails();
            det.desc = n1.getDescriptor(pid);
            det.susp = new Vector<Suspicion>();
            ap.gp.add(det);
          }

        }
      }
      for(i=0;i<ngroups;i++)
      {
        Vector t = new Vector();
        for(j=i;j<Network.size();j+=ngroups)
        {
          Node n1 = Network.get(j);
          t.add(n1);
        }
        v.add(t);
      }    
      first = false;
    }
    else
    {
      //Cleanup
      for(i=0;i<v.size();i++)       
      {
        Vector descvec = (Vector)v.get(i);
        for(j=0;j<descvec.size();j++)
        {
          if(!((Node)descvec.get(j)).isUp())
          {
            descvec.remove(j);
          }
        }
      }
      //Regrouping 
      for(i=0;i<Network.size();i++)
      {
        Node n = Network.get(i);
        RobustnessProtocol ap = (RobustnessProtocol)n.getProtocol(pid);
        int vflag=0;
        if(n.isUp() && ap.sp.size()==0)      //Node which is not grouped yet
        {

          //Find a suitable group
          for(j=0;j<v.size();j++)
          {
            if(((Vector)v.get(j)).size()<maxsize) //Add the group members to ap.sp
            { 
              Vector temp = (Vector)v.get(j); 
              vflag=1;
              for(k=0;k<temp.size();k++)
              {
                Node n1 = (Node)temp.get(k);
                Suspicion susp = new Suspicion();
                susp.desc = n1.getDescriptor(pid);
                susp.suspvalue=0;
                susp.esttime=0;
                susp.lastmsgtime=0;
                susp.first=true;
                susp.initval=-1;
                susp.value=-1;
                susp.state = Suspicion.State.ALIVE;
                ap.sp.add(susp);
                GroupDetails det = new GroupDetails();
                det.desc = n1.getDescriptor(pid);
                det.susp = new Vector<Suspicion>();
                ap.gp.add(det);
                RobustnessProtocol ap1 = (RobustnessProtocol)n1.getProtocol(pid);
                susp = new Suspicion();
                susp.desc = n.getDescriptor(pid);
                susp.suspvalue=0;
                susp.esttime=0;
                susp.lastmsgtime=0;
                susp.first=true;
                susp.initval=-1;
                susp.value=-1;
                susp.state = Suspicion.State.ALIVE;
                ap1.sp.add(susp);
              }
              ((Vector)v.get(j)).add(n);
              break;
            }
          }
          if(vflag==0)
          {
            Vector t = new Vector();
            t.add(n);
            v.add(t);
          }
        }
      }
    }
    if(printgroup==1)        //Print group members
    {
      for(i=0;i<v.size();i++)
      {
        Vector temp = (Vector)v.get(i);
        System.out.println("Group "+i+" : ");
        for(j=0;j<temp.size();j++)
        {
          System.out.print(((Node)temp.get(j)).getID()+" ");
        }
      }
    }
    if(printsusp==1)
    {
      for(i=0;i<Network.size();i++)
      {
        Node n = Network.get(i);
        double val;
        int index=0;
        RobustnessProtocol rp = (RobustnessProtocol)n.getProtocol(pid);
        for(j=0;j<rp.sp.size();j++)
        {
          val=((Suspicion)rp.sp.get(j)).suspvalue;
          System.out.println(CommonState.getTime()/1000+" "+val);
          if(val>=0 && val < 2)
          {
            index=(int)Math.floor(val*100);
          }
          else 
          {
            index=outputsize-1;
          }
          output[index]++;  
        }
      }

      if(CommonState.getTime()==endtime)
      {
        for(i=0;i<outputsize;i++)
        {
          System.out.println((double)((double)i/100.0)+" "+output[i]);
        }
      }
    }
    return false;
  }

}
